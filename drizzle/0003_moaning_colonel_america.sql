ALTER TABLE `atlas_practice_snapshots` ADD `mode` text DEFAULT 'snapshot' NOT NULL;--> statement-breakpoint
ALTER TABLE `atlas_practice_snapshots` ADD `catalog` text DEFAULT '[]' NOT NULL;--> statement-breakpoint
ALTER TABLE `atlas_practice_snapshots` ADD `updated_at` text;
...
CREATE TRIGGER atlas_history_practice_snapshots_update AFTER UPDATE ON atlas_practice_snapshots BEGIN
 INSERT INTO atlas_history (owner,entity,record_id,action,before_json,after_json,created_at) VALUES (NEW.owner,'practice_snapshots','practice','updated',json_object('revision', OLD.revision, 'imported_at', OLD.imported_at, 'source_exported_at', OLD.source_exported_at, 'count', json_array_length(OLD.items), 'mode', OLD.mode, 'lessons', json_array_length(OLD.catalog), 'updated_at', OLD.updated_at),json_object('revision', NEW.revision, 'imported_at', NEW.imported_at, 'source_exported_at', NEW.source_exported_at, 'count', json_array_length(NEW.items), 'mode', NEW.mode, 'lessons', json_array_length(NEW.catalog), 'updated_at', NEW.updated_at),strftime('%Y-%m-%dT%H:%M:%fZ','now'));
END;
