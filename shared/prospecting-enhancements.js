...
(()=>{
  ...
  const css=document.createElement('link');css.rel='stylesheet';css.href=new URL('prospecting-enhancements.css',source).href;document.head.appendChild(css);
  function ready(){
    ...
    exportCSV=function(){
      ...
      const rows=[['Name','LinkedIn','Source','Tier','Score','Title','Company','Stage','FollowUp','Sentiment','Note','AUM']];
      CONTACTS.forEach(c=>rows.push([c.name,c.url,c.tier!==null?'warm':'cold',c.tier??'',c.score??'',c.title,c.co,stg(c),fu(c),c.sentiRaw,note(c),S.aum[c.uid]??'']));
    ...
    function databaseMatches(){
      ...
      return CONTACTS.filter(c=>(!dbStage||stg(c)===dbStage)&&(!dbSrc||(dbSrc==='warm'?c.tier!==null:c.src==='cold'))&&(S.set.peers||!c.peer||dbStage||words.length)&&words.every(w=>normalize([c.name,c.title,c.co,c.email,note(c)].join(' ')).includes(w))).sort((a,b)=>rankOf(b)-rankOf(a));
    }
    ...
    pageBase=function(){
      const list=databaseMatches();return '<div class="card prospecting-filters"><label>Find a prospect<input type="search" id="dbq" placeholder="Name, company, email, or note" value="'+esc(q)+'" oninput="dbSearch(this.value)"></label><label>Stage<select onchange="dbStage=this.value;page=0;ProspectingImprovements.refreshResults()"><option value="">All stages</option>'+stages.map(v=>'<option value="'+v+'"'+(dbStage===v?' selected':'')+'>'+v+'</option>').join('')+'</select></label><label>Source<select onchange="dbSrc=this.value;page=0;ProspectingImprovements.refreshResults()"><option value="">Warm + cold</option><option value="warm"'+(dbSrc==='warm'?' selected':'')+'>Warm only</option><option value="cold"'+(dbSrc==='cold'?' selected':'')+'>Cold only</option></select></label><button class="btn ghost" onclick="ProspectingImprovements.clearFilters()">Clear filters</button><span id="prospecting-result-count" role="status" aria-live="polite">'+list.length+' contacts</span></div><div id="prospecting-results">'+resultsHTML(list)+'</div>';
    };
    ...
    function clearFilters(){q='';dbStage='';dbSrc='';page=0;render(true);document.getElementById('dbq')?.focus();}
    ...
    renderRun=function(){
      ...
      originalRun();const card=document.getElementById('runCard'),copy=card.querySelector('[onclick="runCopy()"]');if(copy)copy.textContent='Copy message';
    ...
    document.addEventListener('keydown',e=>{
      ...
      if(restoreDialog.open||document.getElementById('tplOv').style.display==='grid'||e.target?.closest('button,a[href]')||e.ctrlKey||e.metaKey||e.altKey){e.stopImmediatePropagation();return;}
    ...
    render=function(keep){
      originalRender(keep);const main=document.getElementById('main');labelRows(main);
    ...
    window.ProspectingImprovements=Object.freeze({validateBackup,validateState,databaseMatches,refreshResults,clearFilters,dayQueue,startQueue,csvCell:cell,get unsaved(){return unsaved;},get blocked(){return unreadable||readBlocked;}});
