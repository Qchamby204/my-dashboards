...
(()=>{
  ...
  function start(){
    ...
    function restorePractice(value){
      ...
      curDrill={id:draft.id,drill,topic:copy(draft.topic),scores:draft.scores.slice(),tx:draft.tx,recSecs:draft.recSecs,prepOpen:!!draft.prepOpen,graded:!!draft.graded,gradePaste:draft.gradePaste||'',nextFocus:draft.nextFocus||''};
    ...
    finishDrill=function(){
      ...
      const entry={id:session.id,date:new Date().toISOString(),drill:drill.name,skill:drill.skill,topic:topic.text,score,transcript:session.tx||'',recSecs:session.recSecs||0,rubricScores:scores.slice(),nextFocus:session.nextFocus||''};
    ...
    document.addEventListener('input',event=>{if(event.target.id==='txBox'&&recOn)stopRec();if(curDrill){if(event.target.id==='communication-next-focus')curDrill.nextFocus=event.target.value;if(event.target.id==='txBox')curDrill.tx=event.target.value;if(event.target.id==='gradeBox')curDrill.gradePaste=event.target.value;lastSaveError='';rememberPractice();}});
    document.addEventListener('click',event=>{
      const repeat=event.target.closest?.('[data-repeat-prompt]');if(repeat){repeatPrompt(repeat.dataset.repeatPrompt);return;}
