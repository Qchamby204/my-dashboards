...
export const TABLES={tasks:'atlas_tasks',projects:'atlas_projects',weeks:'atlas_weeks',practice:'atlas_practice_snapshots',ledger:'atlas_ledger',communication:'atlas_communication'};
...
export function parseWorkspace(raw,currentPractice=[],currentLedger=[],currentCommunication=[]) {
  const b=typeof raw==='string'?JSON.parse(raw):raw;
  if(!b||b.app!=='atlas-os'||![1,2,3,4,5].includes(b.version))fail('Choose an Atlas OS workspace export (versions 1–5). Atlas Vault files belong in the original Home app.');
  const lists={tasks:10000,projects:1000,weeks:1000,practice:1,ledger:1,communication:1};
  const input={...b,practice:b.version===1?currentPractice:b.practice,ledger:b.version<4?currentLedger:b.ledger,communication:b.version<5?currentCommunication:b.communication};
  ...
  const out={app:'atlas-os',version:5,exportedAt:stamp(b.exportedAt),projects:[],tasks:[],weeks:[],practice:[],ledger:[],communication:[]};
...
export function changes(before,after){
  return Object.keys(TABLES).map(key=>{
    const id=r=>key==='practice'?'practice':key==='ledger'?'ledger':key==='communication'?'communication':key==='weeks'?r.week_start:r.id;
    const old=new Map(before[key].map(r=>[id(r),r])),next=new Map(after[key].map(r=>[id(r),r]));
    const rows=[];
    for(const [k,r] of next){const previous=old.get(k),action=!previous?'Add':canonical(comparable(previous))!==canonical(comparable(r))?'Replace':'Keep';rows.push({action,title:r.title||r.week_start||(key==='ledger'?'Life Ledger':key==='communication'?'Speaking practice':'Courier practice'),id:k});}
    for(const [k,r] of old)if(!next.has(k))rows.push({action:'Remove',title:r.title||r.week_start||(key==='ledger'?'Life Ledger':key==='communication'?'Speaking practice':'Courier practice'),id:k});
