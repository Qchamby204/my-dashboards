...
export function createHeraldUI({api,getData,getWeek,load,error,toast,esc,downloadJSON,blocked}){
  ...
  function page(){
    ...
    return `<div class="page-heading"><div><span class="eyebrow">An idea with a place to go</span><h1>The Herald, connected.</h1><p>Plan your content and keep its progress in view across devices.</p></div><button class="primary" data-herald-new>＋ New content</button></div>
      <div class="project-toolbar"><button class="secondary" data-herald-import>Import Herald backup</button><a class="inline-link" href="https://qchamby204.github.io/my-dashboards/the-herald.html" >Open full Herald</a><a class="inline-link" href="#review">Open weekly review</a></div>
      <section class="panel"><div class="section-heading"><h2>Your content plan</h2><span>${active.filter(r=>r.stage!=='published').length} in production</span></div><div class="week-switch herald-week"><button class="icon-button" data-week="-7" aria-label="Previous content week">‹</button><span>${esc(pretty(getWeek()))} – ${esc(pretty(addDays(getWeek(),6)))}</span><button class="icon-button" data-week="7" aria-label="Next content week">›</button></div><p>${week.planned.length} planned this week · ${week.published.length} recorded published this week.</p><p class="small">Stages and dates are your records. Saving here does not upload content, schedule a platform post, or obtain approval. The full Herald shares these same items, with scripts and analytics.</p>${undated?`<p class="small">${undated} published ${undated===1?'item has':'items have'} no recorded publication date. Add the actual date in Edit to include it in weekly review.</p>`:''}<div class="filters"><input id="herald-search" aria-label="Search content" placeholder="Find a title or audience…" value="${esc(query)}"><select id="herald-filter" aria-label="Filter content">${Object.entries(filters).map(([id,label])=>`<option value="${id}"${filter===id?' selected':''}>${label}</option>`).join('')}</select></div><div id="herald-list">${rows()}</div></section>`;
  }
  function weekly(){
