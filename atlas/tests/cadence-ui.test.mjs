...
function appHarness(){
 ...
 const context=vm.createContext({...model,missingOccurrences,createWorkUI:()=>({page:()=>'',saving:false}),createConnectUI:()=>({page:()=>'',saving:false}),createCadenceUI:()=>cadence,createBudgetUI:()=>({dirty:false,saving:false,panel:()=>''}),createCommitmentsUI:()=>({page:()=>''}),createAgendaUI:()=>({panel:()=>''}),createSearchUI:()=>({}),createReflectionUI:()=>({dirty:false,saving:false}),createLedgerUI:()=>({dirty:false,saving:false}),createHeraldUI:()=>({dirty:false,saving:false}),createCommunicationUI:()=>({dirty:false,saving:false}),createEditionRefreshUI:()=>({saving:false}),document,window:{addEventListener:(t,f)=>windowEvents.set(t,f)},location:{hash:'#week'},Date,structuredClone,setInterval(){},setTimeout(){},localDay:()=>week,fetch:(path,options)=>new Promise(resolve=>pending.push({path,options,resolve}))});
