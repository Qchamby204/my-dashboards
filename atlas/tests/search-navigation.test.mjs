...
test('actual app opens fresh records by exact ID, including hidden archives, older lessons and earlier weekly reviews without writes',async()=>{
 ...
 for(const [kind,id] of [['task','task'],['project','project'],['lesson','lesson-79'],['habit','habit'],['day','2026-08-03'],['speaking','speaking'],['content','content'],['review','2026-08-03']]){
  ...
  h.context.result={kind,id};const pending=h.run('resolveSearchResult(result)');pending.catch(()=>{});const index=h.pending.length-1;const response={...fresh,week:kind==='review'?{week_start:id,worked:'An earlier review',change:'',revision:7}:null};await h.reply(index,response);const activate=await pending;
...
test('actual app keeps state on unavailable records, failures, cancellation and new drafts, and invalidates earlier refreshes after activation',async()=>{
 ...
 let pending=h.run('resolveSearchResult(result)');pending.catch(()=>{});await h.reply(1);await assert.rejects(pending,/no longer/);assert.equal(h.run('loadedWeek'),week);
 pending=h.run('resolveSearchResult(result)');pending.catch(()=>{});h.pending[2].reject(Error('Offline'));await assert.rejects(pending,/offline/);assert.equal(h.run('data.ledger'),null);
 h.context.signal={aborted:false};pending=h.run('resolveSearchResult(result,signal)');pending.catch(()=>{});h.context.signal.aborted=true;await h.reply(3);await assert.rejects(pending,/cancelled/);
 pending=h.run('resolveSearchResult(result)');pending.catch(()=>{});h.run('reviewDirty=true');await h.reply(4);await assert.rejects(pending,/draft is still here/);const n=h.pending.length;await assert.rejects(h.run('resolveSearchResult(result)'),/current draft/);assert.equal(h.pending.length,n);h.run('reviewDirty=false');
 const oldLoad=h.run('load()');pending=h.run('resolveSearchResult(result)');pending.catch(()=>{});const fresh={...h.state,ledger:{days:[{date:week,note:'Fresh saved note',checked:[]}],habits:[]}};await h.reply(6,fresh);(await pending)();await h.reply(5);await oldLoad;assert.equal(h.run('data.ledger.days[0].note'),'Fresh saved note');
});
