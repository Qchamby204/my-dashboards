...
function harness(){
 ...
 const context=vm.createContext({...model,createWorkUI:()=>({page:()=>'',saving:false}),createConnectUI:()=>({page:()=>'',saving:false}),createCadenceUI:()=>({dirty:false,saving:false,home:()=>'',page:()=>''}),createBudgetUI:()=>({dirty:false,saving:false,panel:()=>''}),createCommitmentsUI:()=>({page:()=>''}),createAgendaUI:()=>({panel:()=>''}),createSearchUI:()=>({}),createReflectionUI:()=>({dirty:false,saving:false,form:()=>'',notes:()=>''}),createHeraldUI:()=>ui('content'),createCommunicationUI:()=>ui('speaking'),createLedgerUI:()=>({dirty:false,saving:false,daily:()=>'',selectDay:id=>opened.push({kind:'day',id}),openHabit:id=>opened.push({kind:'habit',id})}),createEditionRefreshUI:()=>({saving:false,panel:()=>''}),document,window:{addEventListener(){}},location,history:{pushState:(a,b,hash)=>location.hash=hash},Date,structuredClone,console,setInterval(){},setTimeout(){},fetch:(path,options)=>new Promise((resolve,reject)=>pending.push({path,options,resolve,reject})),localDay:()=>week});
