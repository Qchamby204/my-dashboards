...
test('the actual app protects review edits from late refreshes and hides the previous review while changing weeks',async()=>{
  ...
  const context=vm.createContext({...model,createCommitmentsUI:()=>({page:()=>''}),createAgendaUI:()=>({panel:()=>''}),createSearchUI:()=>({}),createReflectionUI,createHeraldUI:()=>({dirty:false,saving:false,weekly:()=>'',page:()=>''}),createCommunicationUI:()=>({dirty:false,saving:false,weekly:()=>'',page:()=>''}),createLedgerUI:()=>({dirty:false,saving:false,daily:()=>''}),createEditionRefreshUI:()=>({saving:false}),document,window:{addEventListener(){}},location:{hash:'#review'},Date,structuredClone,console,setInterval(){},setTimeout(){},fetch:(path)=>new Promise(resolve=>pending.push({path,resolve})),localDay:()=>week});
