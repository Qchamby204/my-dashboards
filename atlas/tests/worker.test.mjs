...
test('practice snapshots whitelist fields, replace removed completions, and reject another owner or stale revision',async()=>{
  ...
  const saved=(await request(db,'/api/export')).data;assert.equal(saved.version,8);assert.deepEqual(saved.practice[0].items,[]);assert.equal(saved.practice[0].owner,undefined);db.close();
});
