...
test('all dashboard entry points have versioned browser and home-screen icons',()=>{
 for(const [page,slug]of Object.entries(catalog)){
  ...
  assert.equal(tags.filter(t=>t.includes('type="image/png"')).length,2,page);
  assert.equal(tags.length,3,page);
  for(const tag of tags){
   ...
   if(file.endsWith('.png')){assert.equal(bytes.subarray(1,4).toString(),'PNG');const size=file.endsWith('-32.png')?32:file.endsWith('-64.png')?64:180;assert.equal(bytes.readUInt32BE(16),size);assert.equal(bytes.readUInt32BE(20),size);assert.ok(tag.includes('sizes="'+size+'x'+size+'"'));}
