...
test('the actual app protects budget drafts from late refresh and navigation, then discards only after an explicit reload',async()=>{
 ...
 const context=vm.createContext({...model,createWorkUI:()=>({page:()=>'',saving:false}),createConnectUI:()=>({page:()=>'',saving:false}),createCadenceUI:()=>({dirty:false,saving:false,home:()=>'',page:()=>''}),createBudgetUI,createCommitmentsUI:()=>({page:()=>''}),createAgendaUI:()=>({panel:()=>''}),createSearchUI:()=>({}),createReflectionUI:()=>({dirty:false,saving:false}),createLedgerUI:()=>({dirty:false,saving:false}),createHeraldUI:()=>({dirty:false,saving:false}),createCommunicationUI:()=>({dirty:false,saving:false}),createEditionRefreshUI:()=>({saving:false}),document,window:{addEventListener:(t,f)=>windowEvents.set(t,f)},location:{hash:'#week'},Date,structuredClone,setInterval(){},setTimeout(){},localDay:()=>week,fetch:(path,options)=>new Promise(resolve=>pending.push({path,options,resolve}))});
