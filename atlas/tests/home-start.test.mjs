...
function harness(){
 ...
 const context=vm.createContext({...model,...work,createConnectUI:()=>({page:()=>'',saving:false}),createCadenceUI:()=>({dirty:false,saving:false,home:()=>''}),createBudgetUI:()=>({dirty:false,saving:false}),createCommitmentsUI:()=>({}),createAgendaUI:()=>({}),createSearchUI:()=>({}),createReflectionUI:()=>({dirty:false,saving:false}),createLedgerUI:()=>({dirty:false,saving:false}),createHeraldUI:()=>({dirty:false,saving:false}),createCommunicationUI:()=>({dirty:false,saving:false}),createEditionRefreshUI:()=>({saving:false}),document,window:{addEventListener(){}},location:{hash:'#today'},Date,structuredClone,setInterval(){},setTimeout(){},localDay:()=>day,
 ...
 const run=s=>vm.runInContext(s,context);run(readFileSync(new URL('../work-ui.mjs',import.meta.url),'utf8').replace(/^import .*;\n/gm,'').replace('export function','function'));run(readFileSync(new URL('../app.js',import.meta.url),'utf8').replace(/^import .*;\n/gm,''));
...
test('empty Today has a clear start without a priority setup or admin panels',async()=>{
 const h=harness();await h.reply(0,{tasks:[],projects:[],week:null});const html=h.node('#content').innerHTML;
 assert.match(html,/Add task/);assert.match(html,/Life Map/);assert.match(html,/The Herald/);assert.match(html,/The Courier/);
 assert.doesNotMatch(html,/Your three priorities|data-add-priority|Weekly review|routine|Connected to Home/);
});
test('a title-only task is dated visibly for today and appears without a separate priority request',async()=>{
 const h=harness();await h.reply(0,{tasks:[],projects:[],week:null});h.run('openCapture()');
 assert.equal(h.node('#task-details').open,false);assert.equal(h.form.elements.due_date.value,day);
 h.form.elements.title.value='Finish the draft';const save=h.submit();const sent=JSON.parse(h.pending[1].options.body);
 assert.equal(sent.due_date,day);assert.equal(sent.focus_day,null);
 await h.reply(1,{id:sent.id},201);await h.reply(2,{tasks:[{...sent,status:'open',focus_date:null,focus_slot:null,revision:1}],projects:[],week:null});await save;
 assert.match(h.node('#content').innerHTML,/Finish the draft/);assert.match(h.node('#content').innerHTML,/For today/);
 assert.equal(h.pending.filter(x=>x.path==='/api/priorities').length,0);
});
