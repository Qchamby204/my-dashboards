...
export async function connectedAssets(root){
  ...
  for(const [kind,file] of [['life-map','life-map.html'],['herald','the-herald.html']]){
    ...
    if(kind==='herald'){
      ...
      script=script.replace(/function syncHTML\(\)\{/,"function syncHTML(){if(window.AtlasConnected)return '<h2>Saved with Atlas</h2><p>This Herald on all your devices uses the same saved records.</p><button onclick=\"openSync()\">Close</button>'; ");
    }else{
    ...
    if(kind==='herald'){script+='\n'+(await readFile(new URL('atlas/herald-records.mjs',root),'utf8')).replace('export function','function')+'\n'+await readFile(new URL('atlas/herald-dashboard.js',root),'utf8');add('/connected/herald-dashboard.css',await readFile(new URL('atlas/herald-dashboard.css',root),'utf8'),'text/css; charset=utf-8');}
    ...
    if(kind==='herald'){
      html=html.replaceAll('Lead Generation Command','Ideas, scripts, and publication').replace('</head>','<link rel="stylesheet" href="/connected/herald-dashboard.css"></head>');
      html=html.replace(/<span class="appbar-pill" id="barPill">[\s\S]*?<input type="file" id="impfile"[^>]*>/,'<details class="hd-backups"><summary>Backups</summary><button type="button" onclick="exportData()">Download backup</button><button type="button" onclick="document.getElementById(\'impfile\').click()">Restore backup</button><input type="file" id="impfile" accept=".json" hidden data-ui-only onchange="importData(event)"></details>');
    }
    html=html.replace('</head>','<link rel="stylesheet" href="/connected/style.css"></head>');
    const bar=`<nav class="connected-toolbar" aria-label="Atlas connected app"><a href="/">← Atlas</a><a href="/apps/${kind==='herald'?'life-map':'herald'}">${kind==='herald'?'Life Map':'The Herald'}</a><p id="connected-status" role="status" aria-live="polite">Opening saved work…</p><a id="connected-import" href="/workspace#connect" hidden>Bring existing work</a><button id="connected-retry" hidden>Retry</button><button id="connected-download" hidden>Download draft</button><button id="connected-reload" hidden>Reload saved work</button><div id="connected-discard" hidden><p>Download your draft first if you want to keep it. Reloading replaces this tab's unsaved changes with saved work.</p><button id="connected-discard-confirm">Discard draft and reload</button><button id="connected-keep">Keep editing</button></div></nav>`;
