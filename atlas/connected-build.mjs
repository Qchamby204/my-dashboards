...
export async function connectedAssets(root){
  ...
  for(const [kind,file] of [['life-map','atlas/life-map-legacy.html']]){
    ...
    const replace=(pattern,value)=>{if(!pattern.test(script))throw Error('Connected storage adapter no longer matches '+kind);script=script.replace(pattern,value);};
      ...
      script+="\nwindow.discardConnectedDraft=()=>{window.LifeMapWorkflow?.discardDraft();view.editor=null;};window.connectedDraftState=()=>window.LifeMapWorkflow?.snapshot()||S;window.acceptConnectedState=next=>{S=next;render();};window.connectedDraftOpen=()=>!!window.LifeMapWorkflow?.pending||!!view.editor||!!view.qa.txt.trim()||!!document.querySelector('#connected-app input:focus,#connected-app textarea:focus');\nstore.get(KEY).then(()=>{const id=new URL(location.href).searchParams.get('record'),p=S.projects.find(x=>x.id===id);if(p){view.editor=Object.assign({kind:'proj'},p);render();}});\n";
    ...
    if(kind==='life-map'){script=script.replaceAll('behavior:"smooth"','behavior:window.matchMedia?.("(prefers-reduced-motion: reduce)").matches?"instant":"smooth"');script+='\n'+(await readFile(new URL('atlas/life-map-records.mjs',root),'utf8')).replace('export function','function')+'\nwindow.LifeMapRecords={validate:validateLifeMapRecords};\n'+(await readFile(new URL('atlas/life-map-workflow-core.mjs',root),'utf8')).replace('export function','function')+'\n'+await readFile(new URL('atlas/life-map-dashboard.js',root),'utf8')+'\n'+await readFile(new URL('atlas/life-map-interactions.js',root),'utf8');add('/connected/life-map-dashboard.css',await readFile(new URL('atlas/life-map-dashboard.css',root),'utf8'),'text/css; charset=utf-8');}
