...
(function(){
  ...
  render=function(){
    app.innerHTML='<div class="lm-dashboard"><header class="lm-header"><div><h1>Life Map</h1><p>Projects and household tasks.</p></div><details><summary>Backups</summary><button type="button" data-act="export">Download backup</button><button type="button" data-act="import">Restore backup</button><input type="file" id="importFile" accept="application/json,.json" hidden></details></header><nav class="lm-tabs" aria-label="Life Map">'+[['projects','Projects'],['chores','Chores'],['timeline','Timeline']].map(function(x){return '<button type="button" data-lm="tab" data-tab="'+x[0]+'" aria-pressed="'+(tab===x[0])+'">'+x[1]+'</button>';}).join('')+'</nav><section class="lm-panel">'+(tab==='projects'?projectPanel():tab==='chores'?chorePanel():horizonView())+'</section>'+editorView()+'</div>';
