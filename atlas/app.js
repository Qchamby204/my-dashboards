...
function todayView(){
  ...
  <aside><section class="panel"><div class="section-heading"><h2>Prepare & reflect</h2></div><a class="inline-link" href="#practice">Open synced practice</a><a class="mini-card" href="${appURL('courier')}" target="_blank" rel="noopener"><span class="mini-icon" aria-hidden="true">▱</span><div><h3>Open your Courier briefing</h3><p>Bring one useful idea into your day.</p></div></a><a class="mini-card" href="${appURL('communication-trainer')}" target="_blank" rel="noopener"><span class="mini-icon" aria-hidden="true">◇</span><div><h3>Rehearse a conversation</h3><p>Make room for a focused practice session.</p></div></a><div class="review-prompt"><p>What would make today feel well spent?</p></div><a class="inline-link" href="#week">Shape the rest of your week</a></section>
...
function render(){
  ...
  if(!loaded)return;
  $('#content').innerHTML=({today:todayView,week:weekView,projects:projectsView,review:reviewView,practice:practiceView,apps:appsView,history:historyView}[view]||todayView)();
...
document.addEventListener('click',async event=>{
  ...
  if(reviewDirty&&event.target.closest('[data-capture],#capture,[data-import],[data-project],[data-action],[data-project-action],[data-new-project],[data-practice-import],[data-practice-manage],[data-lesson-action],[data-restore]')){error('Save your review before changing other records.');return;}
...
window.addEventListener('hashchange',()=>{
  ...
  view=['today','week','projects','review','practice','apps','history'].includes(next)?next:'today';if(view==='today'&&week!==monday(today)){week=monday(today);load();}else render();
});
...
$('#capture').disabled=true;view=['today','week','projects','review','practice','apps','history'].includes(location.hash.slice(1))?location.hash.slice(1):'today';render();load();
