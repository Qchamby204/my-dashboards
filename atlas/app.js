...
import {createConnectUI} from './connect-ui.mjs';
...
const appURL=id=>id==='operations-cadence'?'#cadence':id==='life-map'?'/apps/life-map':id==='life-ledger'?'#ledger':id==='communication-trainer'?'#communication':id==='the-herald'?'/apps/herald':LEGACY_ORIGIN+appById(id).file;
...
const hasDraft=()=>workUI.saving||connectUI.saving||cadenceUI.dirty||cadenceUI.saving||projectDirty||projectSaving||budgetUI.dirty||budgetUI.saving||reviewDirty||reflectionUI.dirty||reflectionUI.saving||ledgerUI.dirty||ledgerUI.saving||editionUI.saving||communicationUI.dirty||communicationUI.saving||heraldUI.dirty||heraldUI.saving;
...
const workUI=createWorkUI({getData:()=>data,getToday:()=>today,api,load,blocked:()=>!loaded||hasDraft()||busy,error,toast,esc,formatDay,editTask:t=>openCapture(null,t),mutateTask:mutate});
const connectUI=createConnectUI({api,load,esc,error,toast,blocked:()=>hasDraft()||busy});
...
function meta(task){
  ...
  return `<div class="task-meta"><a href="${appURL(a.id)}"${appURL(a.id).startsWith('#')||appURL(a.id).startsWith('/')?'':' target="_blank" rel="noopener"'}>${esc(a.name)}</a>${p?`<span>${esc(p.title)}</span>`:''}<span>${duration(task.minutes)}</span>${task.routine_id?'<span>Recurring · '+formatDay(task.occurrence_date)+'</span>':''}${task.due_date?`<span class="${task.status==='open'&&task.due_date<today?'due':''}">${task.due_date<today&&task.status==='open'?'Overdue · ':''}${formatDay(task.due_date)}</span>`:''}</div>`;
}
...
function todayView(){return workUI.page();}
...
function appsView(){
  const active=APPS.filter(a=>a.group!=='Archive'&&a.id!=='operations-cadence');
  return `<div class="simple-page"><div class="page-heading"><div><h1>Your apps</h1><p>Open the tool you need.</p></div></div><div class="app-grid simple-app-grid">${active.map(a=>`<a class="app-card" href="${appURL(a.id)}"${appURL(a.id).startsWith('#')||appURL(a.id).startsWith('/')?'':' target="_blank" rel="noopener"'}><span class="app-icon" aria-hidden="true">${a.icon}</span><div><h3>${esc(a.name)}</h3><p>${esc(a.detail)}</p>${['life-map','the-herald'].includes(a.id)?'<small>Connected to Today</small>':''}</div></a>`).join('')}</div><p class="small">Looking for earlier records? <a class="inline-link" href="#connect">Bring existing work into Atlas.</a></p><a class="inline-link" href="#settings">Optional tools and settings</a></div>`;
}
function settingsView(){
  return `<div class="simple-page"><div class="page-heading"><div><h1>Settings</h1><p>A few things you may need occasionally.</p></div></div>
    <section class="panel setting-list"><h2>Your workspace</h2><button class="setting-row" data-appearance><span><strong>Appearance</strong><small>Light, dark, or follow your device.</small></span><span aria-hidden="true">›</span></button><a class="setting-row" href="#connect"><span><strong>Bring existing work</strong><small>Move earlier Life Map and Herald records here once.</small></span><span aria-hidden="true">›</span></a><button class="setting-row" data-export><span><strong>Download a backup</strong><small>Keep a copy of your Atlas records.</small></span><span aria-hidden="true">↓</span></button><a class="setting-row" href="#history"><span><strong>History and recovery</strong><small>Find a change or restore saved work.</small></span><span aria-hidden="true">›</span></a></section>
    <section class="panel setting-list"><h2>Optional tools</h2><p class="small">Use these only when they help.</p>${[['week','Weekly planner'],['cadence','Repeating tasks'],['review','Weekly notes'],['commitments','Manage tasks and archives'],['practice','Courier practice']].map(([href,label])=>`<a class="setting-row" href="#${href}"><strong>${label}</strong><span aria-hidden="true">›</span></a>`).join('')}</section>
    <details class="settings-notes"><summary>About your data</summary><p>Today uses your tasks, Life Map projects, and Herald content. Open Life Map and The Herald through Atlas to use their shared records. Other original apps keep their own records.</p><p>Your original browser copies remain separate. The one-time transfer brings earlier work here; it does not keep those older copies in sync.</p><a class="inline-link" href="${LEGACY_ORIGIN}" target="_blank" rel="noopener">Original app library</a></details></div>`;
}
const historyLabels={priorities:'Chosen priorities',app_states:'Connected app details',cadence:'Operations Cadence',tasks:'Commitments',projects:'Projects',weeks:'Weekly reviews',practice_snapshots:'Courier practice',practice:'Courier practice',workspace:'Workspace',ledger:'Life Ledger',communication:'Speaking practice',herald:'Herald content plan'};
...
function render(){
  ...
  document.querySelectorAll('[data-view]').forEach(a=>{if(a.dataset.view===(['today','work','commitments'].includes(view)?'today':['settings','connect','history','week','review','cadence'].includes(view)?'settings':'apps'))a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
  ...
  $('#content').innerHTML=({settings:settingsView,work:()=>workUI.allPage(),connect:()=>connectUI.page(),today:todayView,week:weekView,commitments:commitmentsUI.page,projects:projectsView,cadence:cadenceUI.page,review:reviewView,practice:practiceView,ledger:ledgerUI.daily,communication:communicationUI.page,herald:heraldUI.page,apps:appsView,history:historyView}[view]||todayView)();
...
function openCapture(projectId=null,task=null,seed=null){
  ...
  form.elements.week_start.value=task?task.week_start||'':seed?.week_start||week;form.elements.due_date.value=task?(task.due_date||''):(week===monday(today)?today:week);
...
document.addEventListener('click',async event=>{
  if(event.target.closest('[data-appearance]')){document.querySelector('.atlas-appearance-trigger')?.click();return;}
...
$('#task-form').addEventListener('submit',async event=>{
  ...
  try {
    ...
    $('#task-dialog').close();await load();toast(editing?'Task updated.':capturePriorityDay?'Priority saved for today.':'Task added.');
...
window.addEventListener('hashchange',()=>{
  ...
  view=['settings','work','connect','today','week','commitments','projects','cadence','review','practice','ledger','communication','herald','apps','history'].includes(next)?next:'today';if(view==='today')week=monday(today);if(['today','week','cadence'].includes(view))load();else render();
});
...
window.addEventListener('focus',()=>{if(loaded&&!busy&&!hasDraft()&&!document.querySelector('dialog[open]')&&!document.activeElement?.matches?.('input,textarea,select'))load();});
$('#capture').disabled=true;view=['settings','work','connect','today','week','commitments','projects','cadence','review','practice','ledger','communication','herald','apps','history'].includes(location.hash.slice(1))?location.hash.slice(1):'today';render();load();

setInterval(()=>{if(document.visibilityState==='visible'&&!busy&&!hasDraft()&&!document.querySelector('dialog[open]')&&!document.activeElement?.matches?.('input,textarea,select')){if(localDay()!==today){today=localDay();week=monday(today);load();}else if(['today','work'].includes(view))load();}},30000);
