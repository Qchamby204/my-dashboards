...
const appURL=id=>id==='life-map'?'#projects':id==='life-ledger'?'#ledger':LEGACY_ORIGIN+appById(id).file;
...
const hasDraft=()=>reviewDirty||ledgerUI.dirty||ledgerUI.saving;
...
function todayView(){
  ...
  <aside><section class="panel"><div class="section-heading"><h2>Prepare & reflect</h2></div><a class="mini-card" href="#ledger"><span class="mini-icon" aria-hidden="true">▤</span><div><h3>Check in with your day</h3><p>Habits and a little reflection, saved across devices.</p></div></a><a class="inline-link" href="#practice">Open synced practice</a><a class="mini-card" href="${appURL('courier')}" target="_blank" rel="noopener"><span class="mini-icon" aria-hidden="true">▱</span><div><h3>Open your Courier briefing</h3><p>Bring one useful idea into your day.</p></div></a><a class="mini-card" href="${appURL('communication-trainer')}" target="_blank" rel="noopener"><span class="mini-icon" aria-hidden="true">◇</span><div><h3>Rehearse a conversation</h3><p>Make room for a focused practice session.</p></div></a><div class="review-prompt"><p>What would make today feel well spent?</p></div><a class="inline-link" href="#week">Shape the rest of your week</a></section>
...
function practiceView(){
  ...
  return pageHeading('Learning you have used','Practice, carried forward.','Work through a lesson, then record the practice you completed.','<button class="secondary" data-practice-import>Import lessons</button>')+
    `<section class="panel practice-status"><div><h2>${managed?'Synced practice':'Reviewed snapshot'}</h2><p>${managed?'Completions and reopened lessons save privately across devices. Later imports protect your existing choices.':'Bring in a practice pack from Courier, then choose synced practice to record work here.'}</p><p class="small">${p?`Last import ${esc(dateTime(p.imported_at))}. Source export ${p.source_exported_at?esc(dateTime(p.source_exported_at)):'date unavailable'}.${p.updated_at?' Last workspace change '+esc(dateTime(p.updated_at))+'.':''}`:'No practice has been imported yet.'}</p></div>${!managed&&all.length?'<button class="primary" data-practice-manage>Use synced practice</button>':''}</section>
    <p class="small">${all.length} lessons · ${completed.size} completed. New Courier editions need a new practice pack. The original Courier page keeps its own browser copy.</p>
...
function appsView(){
  ...
  return connectionsView()+pageHeading('One suite. Many perspectives.','Your Atlas ecosystem.','Open an existing app, or connect it to a commitment from Capture.')+
    ...
    '<p class="footer-note">Atlas Home is the shared workspace. Life Map projects and Life Ledger open here; the other specialist apps retain their existing addresses and browser records.</p><button class="secondary" data-export>Export my Atlas data</button>';
}
const historyLabels={tasks:'Commitments',projects:'Projects',weeks:'Weekly reviews',practice_snapshots:'Courier practice',practice:'Courier practice',workspace:'Workspace',ledger:'Life Ledger'};
const fieldLabels={habit_count:'Habits',day_count:'Saved days',title:'Title',area:'Life area',status:'Status',mode:'Management mode',lessons:'Practice lessons',due_date:'Due date',week_start:'Planned week',minutes:'Minutes',focus_date:'Priority day',focus_slot:'Priority slot',completed_at:'Completed',archived_at:'Archived',app_id:'Connected app',project_id:'Project',capacity:'Weekly minutes',worked:'What worked',change:'What to change',count:'Practice completions',source_exported_at:'Source backup date'};
...
function historyView(){
  ...
  `<section class="panel"><div class="section-heading"><h2>Keep a copy</h2><button class="secondary" data-export>Download workspace backup</button></div><p>A workspace restore replaces projects, commitments, weekly reviews, practice, and Life Ledger. Review the changes first. Atlas saves a recovery copy before applying them.</p><p class="small">History begins with this upgrade. Original browser apps keep their own data. Downloads contain your personal records; store them somewhere private.</p></section>`+
  ...
  `<section class="panel"><div class="section-heading"><h2>Saved changes</h2><button class="text-button" data-history-refresh>Refresh history</button></div><p class="small">Each save includes its history entry. Earlier updates can be reversed while that record remains unchanged. Practice and Life Ledger entries record counts and dates; use the app controls to edit records or restore a workspace backup to recover its contents. History and recovery copies remain here across workspace restores.</p>${historyData?events.length?events.map(h=>`<details class="history-entry"><summary><span><strong>${esc(h.after?.title||h.before?.title||h.after?.week_start||h.before?.week_start||historyLabels[h.entity])}</strong><span class="small">${esc(historyLabels[h.entity])} · ${esc(h.action)} · ${esc(dateTime(h.created_at))}</span></span></summary>${historyDiff(h)}${h.action==='updated'&&['tasks','projects','weeks'].includes(h.entity)?`<button class="secondary" data-history-undo="${h.seq}">Review previous values</button>`:h.entity==='workspace'?`<button class="secondary" data-checkpoint-export="${esc(h.record_id)}">Download the recovery copy</button>`:'<p class="small">'+(h.action==='created'?'Use the app controls to edit or archive this record.':h.entity==='practice_snapshots'?'Open Practice to review completions, or use a workspace recovery copy.':h.entity==='ledger'?'Open Life Ledger to edit a day, or restore a reviewed workspace backup.':'Use the recovery copy for this workspace restore.')+'</p>'}</details>`).join(''):'<p class="quiet-message">Your next saved change will appear here.</p>':'<p>Loading saved history…</p>'}${historyData?.next?'<button class="secondary" data-history-more>Load earlier changes</button>':''}</section>`;
}
...
document.addEventListener('click',async event=>{
  ...
  const close=event.target.closest('[data-close]');if(close){if(ledgerUI.saving)return;document.getElementById(close.dataset.close).close();return;}
  if(hasDraft()&&event.target.closest('[data-capture],#capture,[data-import],[data-project],[data-action],[data-project-action],[data-new-project],[data-practice-import],[data-practice-manage],[data-lesson-action],[data-restore]')){error('Save or discard your draft before changing other records.');return;}
...
window.addEventListener('hashchange',()=>{
  ...
  view=['today','week','projects','review','practice','ledger','apps','history'].includes(next)?next:'today';if(view==='today'&&week!==monday(today)){week=monday(today);load();}else render();
});
...
$('#capture').disabled=true;view=['today','week','projects','review','practice','ledger','apps','history'].includes(location.hash.slice(1))?location.hash.slice(1):'today';render();load();
