...
const appURL=id=>id==='operations-cadence'?'#cadence':id==='life-map'?'#projects':id==='life-ledger'?'#ledger':id==='communication-trainer'?'#communication':id==='the-herald'?'#herald':LEGACY_ORIGIN+appById(id).file;
...
let loaded=false,view='today',editing=null,draftId=null,capturePriorityDay=null,imports=[],undoAction=null,busy=false,reviewDirty=false,loadNumber=0;
...
const cadenceUI=createCadenceUI({api,getData:()=>data,getWeek:()=>week,getToday:()=>today,load,blocked:()=>!loaded||loadedWeek!==week||hasDraft()||busy,error,toast,esc,formatDay});
const hasDraft=()=>cadenceUI.dirty||cadenceUI.saving||projectDirty||projectSaving||budgetUI.dirty||budgetUI.saving||reviewDirty||reflectionUI.dirty||reflectionUI.saving||ledgerUI.dirty||ledgerUI.saving||editionUI.saving||communicationUI.dirty||communicationUI.saving||heraldUI.dirty||heraldUI.saving;
...
async function load({renderPage=true}={}) {
  ...
  try {
    let next=await api('/api/state?week='+requestedWeek);
    if(requestNumber!==loadNumber || requestedWeek!==week) return;
    if(cadenceUI.dirty||projectDirty||budgetUI.dirty||ledgerUI.dirty||reviewDirty||reflectionUI.dirty||communicationUI.dirty||heraldUI.dirty){$('#save-state').textContent='Unsaved draft';return;}
    if(['today','week','cadence'].includes(view)&&next.cadence&&missingOccurrences(next.cadence,next.tasks,requestedWeek).length&&!document.querySelector('dialog[open]')){
      await api('/api/cadence/plan','POST',{week:requestedWeek,revision:next.cadence.revision});
      if(requestNumber!==loadNumber||requestedWeek!==week)return;
      next=await api('/api/state?week='+requestedWeek);
      if(requestNumber!==loadNumber||requestedWeek!==week)return;
      if(cadenceUI.dirty||projectDirty||budgetUI.dirty||ledgerUI.dirty||reviewDirty||reflectionUI.dirty||communicationUI.dirty||heraldUI.dirty){$('#save-state').textContent='Unsaved draft';return;}
    }
...
async function resolveSavedRecord(result,signal,{taskView='today',taskWeek=monday(today)}={}){
  const destinations={routine:'cadence',task:taskView,project:'projects',lesson:'practice',habit:'ledger',day:'ledger',speaking:'communication',content:'herald',review:'review'};
  ...
  const candidates={routine:next.cadence?.routines,task:next.tasks,project:next.projects,lesson:next.practice?.catalog,habit:next.ledger?.habits,day:next.ledger?.days,speaking:next.communication?.reps,content:next.herald?.items,review:next.week?[next.week]:[]};
  ...
  return ()=>{
    ...
    if(result.kind==='routine')cadenceUI.openRecord(record.id);
...
function meta(task){
  ...
  return `<div class="task-meta"><a href="${appURL(a.id)}"${appURL(a.id).startsWith('#')?'':' target="_blank" rel="noopener"'}>${esc(a.name)}</a>${p?`<span>${esc(p.title)}</span>`:''}<span>${duration(task.minutes)}</span>${task.routine_id?'<span>Recurring · '+formatDay(task.occurrence_date)+'</span>':''}${task.due_date?`<span class="${task.status==='open'&&task.due_date<today?'due':''}">${task.due_date<today&&task.status==='open'?'Overdue · ':''}${formatDay(task.due_date)}</span>`:''}</div>`;
}
...
function briefingView(){
  ...
  return `<section class="panel daily-briefing" aria-labelledby="briefing-title"><div class="section-heading"><h2 id="briefing-title">Saved planning dates</h2><span>${formatDay(today)}</span></div>
    <p class="briefing-lead">${b.priorities.length?`${b.priorities.length} ${b.priorities.length===1?'priority':'priorities'} chosen for today.`:'No priorities chosen yet.'} ${b.overdue.length?`${b.overdue.length} overdue ${b.overdue.length===1?'deadline needs':'deadlines need'} a review.`:b.dueToday.length?`${b.dueToday.length} ${b.dueToday.length===1?'deadline is':'deadlines are'} due today.`:'No overdue or due-today dates among the planning records saved here.'}</p>
    <div class="briefing-grid"><div class="briefing-next"><span class="eyebrow">${b.next?'A place to begin':'Make space'}</span>${b.next?`<h3>${esc(b.next.title)}</h3><p>${esc(b.reason)}${b.next.due?' · '+esc(due(b.next)):''}</p><p class="small">${esc(source(b.next))}</p>${link(b.next)}`:'<h3>No planning action to show yet.</h3><p>Add a priority directly, or review the work already in your original apps.</p><button class="secondary" data-add-priority>Add a priority</button>'}</div>
    ...
    <details class="briefing-sources"><summary>Where this information comes from</summary><p>These dates come from commitments and Life Map projects saved in this private workspace, including generated repeating commitments. They do not represent all of your work.</p><ul><li>${data.tasks.length} saved commitments · ${data.projects.length} Life Map projects.</li><li>Original Life Map records appear only after an import. Imported snapshots refresh only when imported again.</li><li>Original Herald plans and Prospecting follow-ups are not automatically connected to this list. The weekly agenda separately shows Herald dates saved in this workspace.</li></ul><a class="inline-link" href="${LEGACY_ORIGIN}index.html" target="_blank" rel="noopener">Open the original app library</a></details>
...
function todayView(){
  ...
  `<div class="columns"><div><section class="panel focus-panel"><div class="section-heading"><div><h2>Your three priorities</h2><p>${focus.length} of 3 chosen</p></div><button class="primary" data-add-priority${focus.length===3?' disabled':''}>＋ Add a priority</button></div><div class="focus-list">${focus.map((t,i)=>taskRow(t,true,i+1)).join('')}${Array.from({length:3-focus.length},(_,i)=>`<div class="empty-slot"><b>0${focus.length+i+1}</b><span>${i===0?'Use Add a priority to choose what matters today.':'Leave this space open until you need it.'}</span></div>`).join('')}</div><div class="statline"><div><strong>${duration(focus.reduce((s,t)=>s+t.minutes,0))}</strong><span>Priority time</span></div><div><strong>${finished.length}</strong><span>Completed today</span></div><div><strong>${overdue}</strong><span>Past due</span></div></div></section>
  <section class="panel"><div class="section-heading"><div><h2>Other saved commitments</h2><p>Choose for today to make an existing commitment a priority.</p></div><button class="text-button" data-capture>＋ Add</button></div>${waiting.length?waiting.map(t=>taskRow(t)).join(''):empty('No other commitments saved here.','Work in the original apps is not automatically copied into this list. You can add a priority directly above.')}</section>${finished.length?`<section class="panel"><div class="section-heading"><h2>Completed today</h2></div>${finished.map(t=>taskRow(t)).join('')}</section>`:''}</div>
  <aside>${briefingView()}${cadenceUI.home()}<section class="panel"><div class="section-heading"><h2>Prepare & reflect</h2></div><a class="mini-card" href="#ledger"><span class="mini-icon" aria-hidden="true">▤</span><div><h3>Check in with your day</h3><p>Habits and a little reflection, saved across devices.</p></div></a><a class="inline-link" href="#practice">Open synced practice</a><a class="mini-card" href="${appURL('courier')}" target="_blank" rel="noopener"><span class="mini-icon" aria-hidden="true">▱</span><div><h3>Open your Courier briefing</h3><p>Bring one useful idea into your day.</p></div></a><a class="mini-card" href="${appURL('communication-trainer')}"><span class="mini-icon" aria-hidden="true">◇</span><div><h3>Log speaking practice</h3><p>Carry a completed session into your weekly review.</p></div></a><div class="review-prompt"><p>What would make today feel well spent?</p></div><a class="inline-link" href="#week">Shape the rest of your week</a></section>
...
function appsView(){
  ...
  return connectionsView()+pageHeading('One suite. Many perspectives.','Your Atlas ecosystem.','Open an existing app, or connect it to a commitment from Capture.')+
    ...
    '<p class="footer-note">Atlas Home is the shared workspace. Life Map projects, Life Ledger, speaking practice, the Herald content plan, and Operations Cadence open here; the other specialist apps retain their existing addresses and browser records.</p><button class="secondary" data-export>Export my Atlas data</button>';
}
const historyLabels={cadence:'Operations Cadence',tasks:'Commitments',projects:'Projects',weeks:'Weekly reviews',practice_snapshots:'Courier practice',practice:'Courier practice',workspace:'Workspace',ledger:'Life Ledger',communication:'Speaking practice',herald:'Herald content plan'};
const fieldLabels={routine_count:'Routines',active_routines:'Active routines',item_count:'Content items',active_items:'Active content',rep_count:'Saved sessions',active_reps:'Active sessions',feed_checked_at:'Courier last refreshed',habit_count:'Habits',day_count:'Saved days',title:'Title',area:'Life area',status:'Status',mode:'Management mode',lessons:'Practice lessons',due_date:'Due date',week_start:'Planned week',minutes:'Minutes',focus_date:'Priority day',focus_slot:'Priority slot',completed_at:'Completed',archived_at:'Archived',app_id:'Connected app',project_id:'Project',capacity:'Weekly minutes',worked:'What worked',change:'What to change',count:'Practice completions',source_exported_at:'Source backup date'};
...
function historyView(){
  ...
  `<section class="panel"><div class="section-heading"><h2>Keep a copy</h2><button class="secondary" data-export>Download workspace backup</button></div><p>A workspace restore replaces projects, commitments, weekly reviews, Courier practice, Life Ledger, speaking practice, the Herald content plan, and Operations routines. Review the changes first. Atlas saves a recovery copy before applying them.</p><p class="small">History begins with this upgrade. Original browser apps keep their own data. Downloads contain your personal records; store them somewhere private.</p></section>`+
...
function render(){
  ...
  $('#content').innerHTML=({today:todayView,week:weekView,commitments:commitmentsUI.page,projects:projectsView,cadence:cadenceUI.page,review:reviewView,practice:practiceView,ledger:ledgerUI.daily,communication:communicationUI.page,herald:heraldUI.page,apps:appsView,history:historyView}[view]||todayView)();
...
function openCapture(projectId=null,task=null,seed=null){
  ...
  $('#task-dialog-title').textContent=task?'Edit commitment':capturePriorityDay?'Add a priority for today':'Capture a commitment';
  $('#task-priority-note').hidden=!capturePriorityDay;$('#task-details').open=!capturePriorityDay;$('#task-submit').textContent=capturePriorityDay?'Save priority':'Save commitment';
  ...
  $('#task-routine-note').hidden=!task?.routine_id;$('#task-dialog').showModal();form.elements.title.focus();
}
...
document.addEventListener('click',async event=>{
  ...
  if(hasDraft()&&event.target.closest('[data-add-priority],[data-capture],#capture,[data-import],[data-project],[data-action],[data-project-action],[data-new-project],[data-practice-import],[data-practice-manage],[data-lesson-action],[data-edition-check],[data-restore]')){error('Save or discard your draft before changing other records.');return;}
  if(event.target.closest('[data-restore]')){openRestore();return;}
  if(event.target.closest('[data-add-priority]')){openCapture(null,null,{priority:true});return;}
...
$('#task-form').addEventListener('submit',async event=>{
  ...
  try {
    ...
    else await api('/api/tasks','POST',{...values,id:draftId,focus_day:capturePriorityDay});
    $('#task-dialog').close();await load();toast(editing?'Commitment updated.':capturePriorityDay?'Priority saved for today.':'Commitment captured. Choose it for today when you are ready.');
...
window.addEventListener('hashchange',()=>{
  ...
  view=['today','week','commitments','projects','cadence','review','practice','ledger','communication','herald','apps','history'].includes(next)?next:'today';if(view==='today')week=monday(today);if(['today','week','cadence'].includes(view))load();else render();
});
...
$('#capture').disabled=true;view=['today','week','commitments','projects','cadence','review','practice','ledger','communication','herald','apps','history'].includes(location.hash.slice(1))?location.hash.slice(1):'today';render();load();
