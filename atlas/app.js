...
const appURL=id=>id==='life-map'?'#projects':id==='life-ledger'?'#ledger':id==='communication-trainer'?'#communication':LEGACY_ORIGIN+appById(id).file;
...
const reflectionUI=createReflectionUI({api,getData:()=>data,getWeek:()=>week,load,render,error,toast,esc,downloadJSON,capture:seed=>openCapture(null,null,seed),blocked:()=>busy||reviewDirty||ledgerUI.dirty||ledgerUI.saving||editionUI.saving||communicationUI.dirty||communicationUI.saving});
const hasDraft=()=>reviewDirty||reflectionUI.dirty||reflectionUI.saving||ledgerUI.dirty||ledgerUI.saving||editionUI.saving||communicationUI.dirty||communicationUI.saving;
...
async function load({renderPage=true}={}) {
  ...
  try {
    ...
    if(ledgerUI.dirty||reviewDirty||reflectionUI.dirty||communicationUI.dirty){$('#save-state').textContent='Unsaved draft';return;}
...
function appsView(){
  ...
  return connectionsView()+pageHeading('One suite. Many perspectives.','Your Atlas ecosystem.','Open an existing app, or connect it to a commitment from Capture.')+
    ...
    '<p class="footer-note">Atlas Home is the shared workspace. Life Map projects, Life Ledger, and speaking practice open here; the other specialist apps retain their existing addresses and browser records.</p><button class="secondary" data-export>Export my Atlas data</button>';
}
const historyLabels={tasks:'Commitments',projects:'Projects',weeks:'Weekly reviews',practice_snapshots:'Courier practice',practice:'Courier practice',workspace:'Workspace',ledger:'Life Ledger',communication:'Speaking practice'};
const fieldLabels={rep_count:'Saved sessions',active_reps:'Active sessions',feed_checked_at:'Courier last refreshed',habit_count:'Habits',day_count:'Saved days',title:'Title',area:'Life area',status:'Status',mode:'Management mode',lessons:'Practice lessons',due_date:'Due date',week_start:'Planned week',minutes:'Minutes',focus_date:'Priority day',focus_slot:'Priority slot',completed_at:'Completed',archived_at:'Archived',app_id:'Connected app',project_id:'Project',capacity:'Weekly minutes',worked:'What worked',change:'What to change',count:'Practice completions',source_exported_at:'Source backup date'};
...
function historyView(){
  ...
  `<section class="panel"><div class="section-heading"><h2>Keep a copy</h2><button class="secondary" data-export>Download workspace backup</button></div><p>A workspace restore replaces projects, commitments, weekly reviews, Courier practice, Life Ledger, and speaking practice. Review the changes first. Atlas saves a recovery copy before applying them.</p><p class="small">History begins with this upgrade. Original browser apps keep their own data. Downloads contain your personal records; store them somewhere private.</p></section>`+
  ...
  `<section class="panel"><div class="section-heading"><h2>Saved changes</h2><button class="text-button" data-history-refresh>Refresh history</button></div><p class="small">Each save includes its history entry. Earlier updates can be reversed while that record remains unchanged. Courier practice, Life Ledger, and speaking practice entries record counts and dates; use the app controls to edit records or restore a workspace backup to recover its contents. History and recovery copies remain here across workspace restores.</p>${historyData?events.length?events.map(h=>`<details class="history-entry"><summary><span><strong>${esc(h.after?.title||h.before?.title||h.after?.week_start||h.before?.week_start||historyLabels[h.entity])}</strong><span class="small">${esc(historyLabels[h.entity])} · ${esc(h.action)} · ${esc(dateTime(h.created_at))}</span></span></summary>${historyDiff(h)}${h.action==='updated'&&['tasks','projects','weeks'].includes(h.entity)?`<button class="secondary" data-history-undo="${h.seq}">Review previous values</button>`:h.entity==='workspace'?`<button class="secondary" data-checkpoint-export="${esc(h.record_id)}">Download the recovery copy</button>`:'<p class="small">'+(h.action==='created'?'Use the app controls to edit or archive this record.':h.entity==='practice_snapshots'?'Open Practice to review completions, or use a workspace recovery copy.':h.entity==='ledger'?'Open Life Ledger to edit a day, or restore a reviewed workspace backup.':h.entity==='communication'?'Open Master Communicator to edit or archive a session, or restore a reviewed workspace backup.':'Use the recovery copy for this workspace restore.')+'</p>'}</details>`).join(''):'<p class="quiet-message">Your next saved change will appear here.</p>':'<p>Loading saved history…</p>'}${historyData?.next?'<button class="secondary" data-history-more>Load earlier changes</button>':''}</section>`;
}
...
function render(){
  ...
  $('#content').innerHTML=({today:todayView,week:weekView,projects:projectsView,review:reviewView,practice:practiceView,ledger:ledgerUI.daily,communication:communicationUI.page,apps:appsView,history:historyView}[view]||todayView)();
...
document.addEventListener('click',async event=>{
  ...
  const close=event.target.closest('[data-close]');if(close){if(busy||ledgerUI.saving||editionUI.saving||communicationUI.saving||reflectionUI.saving)return;document.getElementById(close.dataset.close).close();return;}
...
window.addEventListener('hashchange',()=>{
  ...
  view=['today','week','projects','review','practice','ledger','communication','apps','history'].includes(next)?next:'today';if(view==='today'&&week!==monday(today)){week=monday(today);load();}else render();
});
...
$('#capture').disabled=true;view=['today','week','projects','review','practice','ledger','communication','apps','history'].includes(location.hash.slice(1))?location.hash.slice(1):'today';render();load();
